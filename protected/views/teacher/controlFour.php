...
<div class="span9" style="width: 1159px;height: 750px;margin-top: -19px;background-color: #f8f4f2">
    ...
    <?php } ?>    
        <div class="stage" style="border-bottom:2px solid #ff0000; "><a href="./index.php?r=teacher/control&indexID=<?php echo $_GET['indexID']; ?>&step=4" class="word" style=" color: #ff0000;">会议公文整理</a></div>
    <?php
        if($nowOnStep == 4){?>
    ...
    <div style="background-color: #fff;height: 600px;margin-top: 20px;width: 1082px;margin-left: 16px">
        <img src="<?php echo IMG_URL_NEW; ?>icon_proof.png" style="position: relative;left: 25px;top: 25px;"/><h3 style="position: relative;left: 61px;top: -18px;width: 180px">会议公文整理</h3>
            <?php
    if ($nowOnStep != 0) {
        if($nowOnStep == 1){?>
      ...
      <?php
            ...
            echo '<div class="currentTag"><span class="wordTag1">当前进行:</span><span class="wordTag2">会议公文整理</span></div>';
        }else if($nowOnStep == 5){
